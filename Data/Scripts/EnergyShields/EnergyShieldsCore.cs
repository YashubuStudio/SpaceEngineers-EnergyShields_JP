...
namespace Cython.EnergyShields
{
	...
	public class EnergyShieldsCore: MySessionComponentBase
	{
		...
		void missileCollisionHandler(IMyMissile missile)
		{
			...
			if(grid != null && missile.CollisionPoint.HasValue)
			{
				Vector3I blockPos = grid.WorldToGridInteger(missile.CollisionPoint.Value);
				IMySlimBlock block = grid.GetCubeBlock(blockPos);

				if(block == null) {
					blockPos = grid.WorldToGridInteger(missile.CollisionPoint.Value - missile.CollisionNormal * 0.4f);
				...
				MyDamageInformation damageInformation = new MyDamageInformation();
				damageInformation.AttackerId = missile.LauncherId;
				damageInformation.Amount = missile.HealthPool;
				damageInformation.IsDeformation = true;
				damageInformation.Type = MyStringHash.GetOrCompute("Missile");

				shieldDamageHandler(block, ref damageInformation);

				missile.HealthPool = damageInformation.Amount;
			}
		}
